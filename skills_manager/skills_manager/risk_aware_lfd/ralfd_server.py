...
class _BranchRequested(Exception):
    """The operator pressed "e" during a recovery hold."""
...
class RALfDServer(LfDServer):
    ...
    def _execute_part(self, goal_handle, parts, index, part):
        ...
        self.call_motion_service(
            self.retrain_client, StringService.Request(text=part.name),
        ...
        try:
            ...
            while self.time_index < self.loaded_trajectory_len and not self.end:
                ...
                if anomaly:
                    ...
                    return "play", target
        except _BranchRequested:
            self.wait_for_hold()
            self.begin_motion()  # drop the hold target; the operator takes over
    ...
    def _check_recovery_stop(self):
        self.check_motion()  # cancellation still wins
        if self._branching and self.end:
            raise _BranchRequested()
